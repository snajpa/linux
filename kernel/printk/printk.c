 ...
 */
static int logbuf_has_space(u32 msg_size, bool empty, struct syslog_namespace *ns)
{
	u32 free;

	if (ns->log_next_idx > ns->log_first_idx || empty)
		free = max(ns->log_buf_len - ns->log_next_idx, ns->log_first_idx);
	else
...
static int log_make_free_space(u32 msg_size, struct syslog_namespace *ns)
{
	...
	if (logbuf_has_space(msg_size, ns->log_first_seq == ns->log_next_seq, ns))
		return 0;
		     	...
		     	struct syslog_namespace *ns)
{
	...
	if (ns->log_next_idx + size + sizeof(struct printk_log) > ns->log_buf_len) {
		/*
		 * This message + an additional empty header does not fit
		 * at the end of the buffer. Add an empty header with len == 0
		 * to signify a wrap around.
		 */
		memset(ns->log_buf + ns->log_next_idx, 0, sizeof(struct printk_log));
		ns->log_next_idx = 0;
	}
...
static bool cont_add(int facility, int level, enum log_flags flags, const char *text, size_t len, struct syslog_namespace *ns)
{
	/*
		...
		size_t text_len, struct syslog_namespace *ns)
{
	...
	if (cont.len) {
		if (cont.owner == current && (lflags & LOG_CONT)) {
			if (cont_add(facility, level, lflags, text, text_len, ns))
				return text_len;
		}
	...
	return log_store(facility, level, lflags, 0, dict, dictlen, text, text_len, ns);
}
		  ...
		  const char *fmt, va_list args)
{
	...
	char *text = textbuf;
	size_t text_len;
	enum log; enum log_flags lflags = 0;
...
static int ns_vprintk_emit(struct syslog_namespace *ns, int facility, int level,
			const char *dict, size_t dictlen,
			const char *fmt, va_list args)
{
	int printed_len;
	...
	logbuf_lock_irqsave(flags, ns);
	printed_len = ns_vprintk_store(ns, facility, level, dict, dictlen, fmt, args);
	logbuf_unlock_irqrestore(flags, ns);
...
asmlinkage int vprintk_emit(int facility, int level,
				...
				const char *fmt, va_list args)
{
	return ns_vprintk_emit(&init_syslog_ns, facility, level, dict, dictlen, fmt, args);
}
...
asmlinkage int ns_printk_emit(struct syslog_namespace *ns, int facility, int level,
			   const char *dict, size_t dictlen,
			   const char *fmt, ...)
{
	va_list args;
	...
	for (;;) {
		...
		if (ns->console_seq < ns->log_first_seq) {
			len = sprintf(text, "** %u printk messages dropped **\n",
				      (unsigned)(ns->log_first_seq - ns->console_seq));
